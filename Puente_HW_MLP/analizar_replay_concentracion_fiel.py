...
PRED_CSV = Path("predictions_log_concentracion_fiel.csv")
TIMELINE_CSV = Path("replay_timeline_concentracion_fiel.csv")
FS = 500.0
...
print(f"Accuracy temporal frente al replay fiel esperado: {acc:.4f}")
...
plt.xlabel("Tiempo (s)")
plt.ylabel("Valor")
plt.title("Probabilidad predicha vs replay fiel esperado")
plt.ylim(-0.05, 1.05)
plt.legend()
plt.tight_layout()
plt.savefig("replay_fiel_probabilidad_vs_estado.png", dpi=200)
plt.show()
...
plt.xlabel("Tiempo (s)")
plt.ylabel("Clase")
plt.title("Clase predicha vs replay fiel esperado")
plt.yticks([0, 1], ["Reposo", "Cálculo mental"])
plt.legend()
plt.tight_layout()
plt.savefig("replay_fiel_clase_vs_estado.png", dpi=200)
plt.show()